...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  # The crewmate launch drops the USER settings source and loads a firstmate-derived
  # copy instead, so an operator permissions.ask rule cannot park the pane
  # (bin/fm-spawn.sh's claude launch_template branch owns why).
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --setting-sources project,local --settings '$HOME_DIR/state/$id.claude-settings.json' \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
...
test_claude_threads_model_and_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "claude --dangerously-skip-permissions --setting-sources project,local --settings '$HOME_DIR/state/$id.claude-settings.json' --model 'sonnet' --effort 'high'" \
...
test_claude_forwards_firstmate_config_dir_when_set() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$CASE_DIR/claude-work' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --setting-sources project,local" \
