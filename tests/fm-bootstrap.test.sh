...
test_network_phase_partitions_the_run() {
  ...
  #
  # Hide node the way the missing-git case hides git. Removing the fake binary
  # is not enough: BASE_PATH keeps /usr/bin, and a machine with a distro node
  # there still resolves one, so the local diagnostic never fires and the case
  # fails for a reason that has nothing to do with the partition.
