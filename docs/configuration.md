...
`state/` is captain-private in mode as well as in git: `fm_private_dir_ensure` in `bin/fm-wake-lib.sh` owns creating it owner-only whatever umask the operator happens to have, and every other creator calls that helper rather than `mkdir`.
A state root an earlier run left group- or world-writable is tightened on the next run, which is what keeps `bin/fm-procevent.sh` from refusing the home as a non-private state root.
