...
`state/` is captain-private in mode as well as in git: `fm_private_dir_ensure` in `bin/fm-wake-lib.sh` creates it owner-only whatever umask the operator happens to have, and every script that sources `bin/fm-wake-lib.sh` runs that helper on its own state root at source time.
Only that directory is made private: a missing home above it is created on the operator's own umask, because the privacy contract covers the state root and nothing around or above it, including `config/`.
Other creators still use a plain `mkdir`, so a home seeded or propagated into by `bin/fm-home-seed.sh`, `bin/fm-bootstrap.sh`, or `bin/fm-config-push.sh` can start out group-writable under a permissive umask; the helper tightens a state root an earlier run left group- or world-writable on the next run that sources the library, which is what keeps `bin/fm-procevent.sh` from refusing the home as a non-private state root.
