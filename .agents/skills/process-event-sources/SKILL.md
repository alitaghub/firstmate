...
: A `telegram` wake means the channel is broken rather than that a message arrived, because an applied capture announces itself through the notes it queued. `bin/fm-procevent-telegram.sh classify <result-file>` returns `updates`, `unreachable`, `error`, or `malformed`; an `error` result names the cause - a refused token, a second reader on the same bot, or a removed allowlist - and the source is deliberately left unarmed, so decide the fix and re-arm with `bin/fm-procevent-telegram.sh arm`. An `unreachable` result is a failure that fixes itself - no route, a `429`, a `5xx`, or a body that is not Bot API JSON - so the adapter re-arms and acknowledges it silently and no wake reaches you; only a refusal Telegram actually returned (`401`, `409`) becomes an `error`. Never acknowledge a `telegram` wake through the generic command before the adapter's `handle` has run.
