...
The captain's Telegram channel, when this home has opted in, is armed and handled through `bin/fm-procevent-telegram.sh`.
Its header owns the exact commands; the adapter owns the allowlist check, note queueing, offset continuity, acknowledgement, and re-arming after a capture.
It deliberately has no `answers` command, so a Telegram message can never close a captain-held task: it only ever becomes a captain inbox note that firstmate reads and judges.
...
`bin/fm-procevent.sh --help`, `bin/fm-procevent-lavish.sh --help`, `bin/fm-procevent-when.sh --help`, `bin/fm-procevent-quota.sh --help`, `bin/fm-procevent-telegram.sh --help`, and `bin/fm-procevent-remote-reply.sh --help` own the exact commands and flags.
...
: A `telegram` wake means the channel is broken rather than that a message arrived, because an applied capture announces itself through the notes it queued. `bin/fm-procevent-telegram.sh classify <result-file>` returns `updates`, `unreachable`, `error`, or `malformed`; an `error` result names the cause - a refused token, a second reader on the same bot, a removed allowlist, or a window the poll can never advance past - one whose every update id the read position has already passed, since the position moves past a whole window including the updates the allowlist dropped - and the source is deliberately left unarmed, so decide the fix and re-arm with `bin/fm-procevent-telegram.sh arm`. An `unreachable` result is a failure that fixes itself - no route, a `429`, a `5xx`, a body that is not Bot API JSON, or empty windows that never hold the long poll open - so the adapter re-arms and acknowledges it silently and no wake reaches you; anything that will not fix itself becomes an `error` instead. Never acknowledge a `telegram` wake through the generic command before the adapter's `handle` has run.
